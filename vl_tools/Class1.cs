...
namespace vl_tools
{
    ...
    public class Commands : IExtensionApplication
    {
                    ...
                    switch (KwRes2.StringResult)
                    {
                        ...
                        case "Показать":
                            ...
                            using (Transaction trans = db.TransactionManager.StartTransaction())
                            {
                                ...
                                VL_UtilityClass.ViewEntityPos(oid);
                                trans.Commit();
                            } // using
                            break;
                    }
                    ...
                    foreach (IGrouping<string, Abonent> g in TPAbGroups)
                    {
                        Transaction trans = dbCurrent.TransactionManager.StartTransaction();
                        {
                            try
                            {
                                ...
                                if (cntr == 1)
                                {
                                    lt = VL_UtilityClass.ImportLayoutWithOrWithoutReplace(_TemplatePath + "template.dwg", "Таблица ТП");
                                    curName = "Таблица ТП";
                                }
                                else
                                {
                                    lt = VL_UtilityClass.ImportLayoutWithOrWithoutReplace(_TemplatePath + "template.dwg", "Таблица ТП л.2");
        ...
        {
            VL_UtilityClass.ImportDwg("D:\\ВР32-31.DWG");
        }
        ...
        [CommandMethod("VL_ROAD_LIGHT")]
        public void cmdVL_ROAD_LIGHT()         
        {
            //Расстановка опор освещения
            try
            {
                Editor ed = Autodesk.AutoCAD.ApplicationServices.Application.DocumentManager.MdiActiveDocument.Editor;
                Database dbCurrent = HostApplicationServices.WorkingDatabase;
                Polyline pl1 = VL_UtilityClass.GetPolyline("Выберите полилинию для расстановки опор");

                Polyline _pl1 = pl1.Clone() as Polyline;
                _pl1.Elevation = 0;

                //Get  point                        
                PromptPointOptions prPntOpt = new PromptPointOptions("\nУкажите точку начала расстановки: ");
                PromptPointResult prPntRes = ed.GetPoint(prPntOpt);
                if (prPntRes.Status != PromptStatus.OK) { ed.WriteMessage("\nПрограмма отменена пользователем."); return; }
                Point3d startPt = prPntRes.Value;
                //Transform to WCS and set z coordinate t0 null                    
                Point3d PntWCS = startPt.TransformBy(ed.CurrentUserCoordinateSystem);
                Point3d startPnt2D = new Point3d(PntWCS.X, PntWCS.Y, 0);

                PromptDoubleOptions dblOpt = new PromptDoubleOptions("\nВведите шаг опор:");
                dblOpt.DefaultValue = 35;
                PromptDoubleResult dblRes = ed.GetDouble(dblOpt);
                if (dblRes.Status != PromptStatus.OK) return;
                double step = dblRes.Value;
